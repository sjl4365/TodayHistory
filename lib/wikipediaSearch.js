import { fetchImageForContent } from './googleSearch';// Adjust path as needed
...
export async function fetchImageWithFallback(searchQuery, anchorTexts = null, language = 'en') {
  console.log('🖼️ Starting image search with fallback...');
  
  let imageUrl = null;

  console.log('📚 Trying Wikipedia...');
  if (anchorTexts && Array.isArray(anchorTexts) && anchorTexts.length > 0) {
    imageUrl = await fetchWikipediaImageFromAnchors(anchorTexts, language);
  } else {
    imageUrl = await fetchWikipediaImage(searchQuery, language);
  }
  
  if (!imageUrl) {
    console.log('⚠️ Wikipedia failed, trying Google Images...');
    imageUrl = await fetchImageForContent(searchQuery);
  } else {
